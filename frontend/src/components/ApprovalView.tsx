  ...
  if (!currentProposal) {
    return (
      <div className="bg-white border border-slate-200 rounded-xl p-12 text-center space-y-4 shadow-sm">
        ...
        <h3 className="text-lg font-bold text-slate-900">Không Có Đề Xuất Nào Chờ Duyệt</h3>
        <p className="text-xs text-slate-500">Tất cả sự cố đã được giải quyết hoặc chưa có đề xuất mới sinh từ PuLP MILP Solver.</p>
        <button
          onClick={onBack}
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-3">
          <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider">Danh Sách Đề Xuất AI</h3>
              ...
              return (
                ...
                >
                  ...
                  <div className={`text-[11px] flex items-center justify-between ${isSelected ? "text-slate-300" : "text-slate-500"}`}>
                    ...
                    <span className="font-mono font-bold">{p.rankings?.length || 0} Đối tác</span>
                  </div>
                </button>
              );
            })}
          ...
          <div className="bg-white border border-slate-200 p-5 rounded-xl shadow-sm space-y-3.5">
            ...
            <div className="grid grid-cols-3 gap-3 bg-slate-50 p-3 rounded-lg border border-slate-200 text-xs">
              ...
              <div>
                ...
                <span className="font-mono font-bold text-slate-900">{currentProposal.quantity} đơn vị</span>
              </div>
            </div>
            ...
            {currentProposal.recommendation && (
              <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg text-xs text-slate-800 flex items-start gap-2.5">
                ...
                <div>
                  <span className="font-bold text-slate-900 block mb-0.5">Đề Xuất Kiến Nghị Từ AI Engine (GPT-4o Reasoning):</span>
          ...
          <div className="space-y-4">
            ...
            {currentProposal.rankings?.map((item, idx) => (
              ...
              >
                <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <span className="text-[11px] text-slate-500 font-mono">Mã NCC: {item.supplierId}</span>
                    </div>
                  </div>

                  <div className="text-right">
                    <div className="text-[11px] text-slate-500">Điểm Tối Ưu Score</div>
                ...
                <div className="grid grid-cols-3 gap-3 bg-slate-50 p-3 rounded-lg border border-slate-200 text-xs">
                  <div>
                    <span className="text-slate-500 block text-[11px]">Đơn Giá Đề Xuất</span>
                    <span className="font-mono font-bold text-emerald-700">
                      {Number(item.unitPrice).toLocaleString("vi-VN")}đ
                    </span>
                  </div>
                  <div>
                    <span className="text-slate-500 block text-[11px]">Thời Gian Giao (LeadTime)</span>
                    <span className="font-mono font-bold text-slate-900">{item.leadTimeDays} ngày</span>
                  </div>
                  <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
                  <div className="bg-emerald-50/60 border border-emerald-200 p-3 rounded-lg space-y-1">
                    <div className="font-bold text-emerald-800 flex items-center gap-1">
                      <ThumbsUp className="w-3.5 h-3.5 text-emerald-600" /> Ưu Điểm Nổi Bật:
                    </div>
                  ...
                  <div className="bg-red-50/60 border border-red-200 p-3 rounded-lg space-y-1">
                    <div className="font-bold text-red-800 flex items-center gap-1">
                      <ThumbsDown className="w-3.5 h-3.5 text-red-600" /> Nhược Điểm & Rủi Ro:
                    </div>
                    <ul className="space-y-0.5 text-slate-700">
